...
public class QuarryRenderer implements BlockEntityRenderer<QuarryBlockEntity> {
    ...
    private static final double STILL_SEG_EPS = 1.0e-4;
    private static final int STILL_TICKS_TO_ARM = 3;
    ...
    public void render(QuarryBlockEntity be, float partialTick, PoseStack pose,
        ...
        double u = tClamped * tClamped * (3.0 - 2.0 * tClamped);
        Vec3 seg = anim.to.subtract(anim.from);

        Vec3 dispW = (tRaw <= 1.0) ? anim.from.add(seg.scale(u)) : anim.to;
        ...
        if (mineableHere && (anim.slamStartGT >= 0 || anim.slammedThisCell || anim.stillTicks >= STILL_TICKS_TO_ARM)) {
            cxW = wx + 0.5; czW = wz + 0.5; dispW = anim.to;
        }
        if (mineableHere && anim.slamStartGT < 0 && !anim.slammedThisCell && anim.stillTicks >= STILL_TICKS_TO_ARM) {
            anim.slamStartGT = gt; anim.slammedThisCell = true;
        }
        ...
        if (anim.slamStartGT >= 0) {
            ...
            if (dt <= SLAM_DOWN_T) slamDipBlocks = SLAM_DIP_BLOCKS * easeOutCubic(dt / SLAM_DOWN_T);
            else if (dt <= SLAM_DOWN_T + SLAM_HOLD_T) slamDipBlocks = SLAM_DIP_BLOCKS;
            else if (dt <= SLAM_TOTAL)
                slamDipBlocks = SLAM_DIP_BLOCKS * (1.0 - easeOutCubic((dt - SLAM_DOWN_T - SLAM_HOLD_T) / SLAM_UP_T));
            else anim.slamStartGT = -1;
        }
        ...
        // --- Vertical riser: full blocks + thin cap to just touch the diamond ---
        int extraRiser = (int)Math.floor(slamDipBlocks + 1.0e-6);
        int bottomInclusive = tipLocalYi + 1 - extraRiser;

        int fullBottom = bottomInclusive + 1;
        for (int y = yTopLocalI; y >= fullBottom; y--) {
            pose.pushPose();
        ...
        // 4-pixel lip that barely touches diamond tip
        double capH = 0.25;
        int yLip = bottomInclusive;
        pose.pushPose();
        pose.translate(cx - 0.5, yLip + (1.0 - capH) + 0.05, cz - 0.5);
        pose.scale(1.0f, (float)capH, 1.0f);
        BlockPos posLip = base.offset((int)(cx - 0.5), yLip, (int)(cz - 0.5));
        brd.renderSingleBlock(riser, pose, buffers, LevelRenderer.getLightColor(level, posLip), packedOverlay);
        pose.popPose();
